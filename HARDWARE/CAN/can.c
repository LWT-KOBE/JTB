...
//返回值:0,成功;
//		 其他,失败;
u8 Can_Send_Msg(u8 len)  //申请地址
{
	u32 ReportResults,j = 0;
	u8 checksum = 0;
	u8 mbox;
	u16 i=0;

	CanTxMsg TxMessage;
	TxMessage.StdId=TrainBasketID;			// 帧ID,也是车厢号 
	TxMessage.IDE=CAN_Id_Standard; 	// 标准帧
	TxMessage.RTR=CAN_RTR_Data;		// 数据帧
	TxMessage.DLC=len;				// 要发送的数据长度
	
	TxMessage.Data[0] = Applictiao_ADDR_Flag;     //  
	TxMessage.Data[1] = LastDeliverResult>>8;   
	TxMessage.Data[2] = LastDeliverResult; 		
	TxMessage.Data[3] = BasketErrorFlag;			//故障码
	
	TxMessage.Data[4] = PocketCount>>8;				//PocketCount
	TxMessage.Data[5] = PocketCount;				//PocketCount
	
	TxMessage.Data[6] = CaseNum; //固件版本号
	TxMessage.Data[7] = CageNumber; //固件版本号
...
u8 Can_Send_LastDeliverResult(u8 len)  //上报投递结果
{
	...
	u16 i=0;
	CanTxMsg TxMessage;
	TxMessage.StdId=(TrainBasketID & 0xff) | 0x100;			// 帧ID,也是车厢号 
	TxMessage.IDE=CAN_Id_Standard; 	// 标准帧
	TxMessage.RTR=CAN_RTR_Data;		// 数据帧
	TxMessage.DLC=len;				// 要发送的数据长度
	TxMessage.Data[0] = LastDeliverResult_Flag;     //0x01上报投递结果  
	TxMessage.Data[1] = LastDeliverResult>>8;     //上报投递结果位置2BYTE B1*256+B2
	TxMessage.Data[2] = LastDeliverResult; 		//上报投递结果位置2BYTE B1*256+B2
	TxMessage.Data[3] = BasketErrorFlag;			//故障码
	TxMessage.Data[4] = PocketCount>>8;				//PocketCount
	TxMessage.Data[5] = PocketCount;				//PocketCount
	TxMessage.Data[6] = 0;	
